...
public class WorldDB {
    ...
    private static final String GET_CITIES_COUNTRY =
            "SELECT * FROM city " +
                    ...
                    "WHERE country.code = ?";
    private static final String GET_LANGUAGE_COUNTRY =
            "SELECT * FROM language WHERE countrycode = ?"
    ...
    public List<Language> getLanguageByCode() {
        List<Language> countries = new ArrayList<>();
    //refactor down to base off language
        try {
            Statement sql = this.conn.createStatement(GET_LANGUAGE_COUNTRY);
            ResultSet results = sql.executeQuery(GET_LANGUAGE_COUNTRY);
